...
- ~~Toggle para el inglés en las tarjetas (que no aparezca de una).~~ Botón `EN` en el reverso. El default es OCULTO EN EL CSS (`.significados, .traduccion { visibility: hidden }`) y el tap agrega `en-on` para revelar: ocultarlo por JS al cargar dejaba un parpadeo del inglés antes de que corriera el script. Botón propio y no tap sobre la carta entera porque en AnkiDroid ese gesto ya está tomado. Contrapartida asumida: sin JS el inglés queda inalcanzable.
