...
| F    | app/ — recortes/notas: la captura crea su propio tipo de contenido en vez de una historia | ✅ Completo (PR pendiente — actualizar con #N al abrir; smoke parcial en dispositivo 2026-09-17, ver `docs/smoke-recortes.md`) |
...
- **Recortar sobre la imagen ya guardada y re-correr el OCR.** Ataca el mismo problema
  desde el otro lado y es el más barato de los cuatro: el `.jpg` ya está en la nota, así
  que es recortar + re-OCR + regenerar párrafos, todo con piezas existentes
  (`OcrJapones`, `escalarRecorte`, `CreadorRecortes.editar`).
