...
- **OJO con el build release**: R8 rompe ML Kit si no están las keep rules de
  `proguard-rules.pro`. El síntoma es traicionero porque **no crashea**: `OcrJapones` deja
  propagar la `NullPointerException` y el Service la sustituye por texto vacío, así que cada
  captura devuelve `OCR devolvió 0 chars` y la feature queda muda. Se detecta rápido por el
  tiempo: el OCR sano tarda ~300-500 ms, el roto vuelve en ~13 ms. Es el mismo cuadro que ya
  había pasado con Kuromoji ("Could not load dictionaries"). **Probar el APK release en
  dispositivo antes de publicar cualquier release, no alcanza con que compile.**
